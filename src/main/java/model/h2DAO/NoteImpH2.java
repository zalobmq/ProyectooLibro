...
public class NoteImpH2 {
	 ...
	 * 
	 * @Return Booleano que devuelve verdadero si se ha borrado la nota y falso si
	 ...
	 * @Param Note n
	 * 
	 * @Return booleano que devuelve verdadero si se ha conseguido guardar la nota y
	 * falso si ha fallado
	 */
	 ...
	 * nota y falso si ha fallado
	 */
	public static boolean update(Note n) throws DAOException {
		boolean result = false;
		EntityManager em = createEM();
		try {
			n = em.merge(n);// se relaciona el objeto de java con su tupla en la base de datos para
							// actualizarla
			em.getTransaction().begin();
			em.getTransaction().commit();
	 ...
	 * 
	 * @Return Note que tiene la id correspendiente en caso de que se haya
	 * encontrado o id=-1 si la nota no se ha encontrado
	 * 
	 */
	...
	/*
	 * Método que se usa para buscar notas por el título
	 * 
	 * @Param String title con el título de la nota que se desa buscar
	 * 
	 * @Return List<Note> con las notas que coincidan con dicho nombre, en caso de
